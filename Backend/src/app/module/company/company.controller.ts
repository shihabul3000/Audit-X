...
const updateCompany = catchAsync(async (req: Request, res: Response) => {
  ...
  const result = await CompanyService.updateCompany(id, req.body.name, req.user!.userId, req.user!.role);
...
const deleteCompany = catchAsync(async (req: Request, res: Response) => {
  const id = req.params['id'] as string;
  const result = await CompanyService.deleteCompany(id, req.user!.userId, req.user!.role);
...
const assignCompany = catchAsync(async (req: Request, res: Response) => {
  const id = req.params['id'] as string;
  const result = await CompanyService.assignCompanyToUser(id, req.body.userId);
...
const unassignCompany = catchAsync(async (req: Request, res: Response) => {
  const id = req.params['id'] as string;
  const result = await CompanyService.unassignCompanyFromUser(id, req.body.userId);
