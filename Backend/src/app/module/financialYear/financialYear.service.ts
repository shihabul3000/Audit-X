...
export const createFinancialYear = async (
  ...
  const existing = await prisma.financialYear.findFirst({
    where: { companyId, year: yearNumber },
  });
  if (existing && !existing.isDeleted) throw new AppError(status.CONFLICT, `Financial year ${yearNumber} already exists for this company`);
  ...
  if (prevYear && prevYear.auditData && prevYear.notesData) {
    const rolled = performRollover(
      prevYear.auditData.data as unknown as Parameters<typeof performRollover>[0],
      prevYear.notesData.data as unknown as Parameters<typeof performRollover>[1],
      reportingDate,
...
const VALID_TRANSITIONS: Record<string, string[]> = {
  DRAFT: ['SUBMITTED'],
  SUBMITTED: ['UNDER_REVIEW', 'CHANGES_REQUESTED', 'FINALIZED', 'DRAFT'],
  UNDER_REVIEW: ['CHANGES_REQUESTED', 'FINALIZED', 'DRAFT'],
...
export const submitFinancialYear = async (yearId: string, userId: string, note?: string) => {
  ...
  console.log(`[SUBMIT] yearId=${yearId} currentStatus=${year.reviewStatus} userId=${userId}`);
