...
const syncToBackend = (yearId: string, type: 'auditData' | 'notesData' | 'discussionData', data: any) => {
  if (timeouts[`${yearId}-${type}`]) {
    clearTimeout(timeouts[`${yearId}-${type}`]);
  }
  timeouts[`${yearId}-${type}`] = setTimeout(() => {
    import('../services/financialData.service').then(({ financialDataService }) => {
      if (type === 'auditData') {
        financialDataService.updateAuditData(yearId, data).catch(console.error);
      } else if (type === 'notesData') {
        financialDataService.updateNotesData(yearId, data).catch(console.error);
      } else if (type === 'discussionData') {
        financialDataService.updateDiscussionData(yearId, data).catch(console.error);
      }
    });
  }, 1500);
};
...
export const permissionSelectors = {
  canAccessCompany: (user: User | null, company: Company | null) => {
    ...
    // For students: the backend already filters companies to only assigned ones,
    // so if it appears in the list, the user has access. Also check assignedCompanyIds
    // as a fallback (may be undefined if not populated from backend).
    return company.createdByUserId === user.id || (user.assignedCompanyIds || []).includes(company.id);
  },
    ...
    immer((set, get) => ({
      ...
      fetchCompanies: async () => {
        try {
          ...
          // httpClient interceptor unwraps axios response.data, so response = { message, statusCode, data: { data: [...], meta } }
          const companiesArray = response?.data?.data || response?.data || [];
          set(state => {
            state.companies = (Array.isArray(companiesArray) ? companiesArray : []).map((c: any) => ({
              ...c,
      ...
      updateActiveYearAuditData: (updater: (draft: AuditReportData) => void) => {
        set(state => {
          const company = state.companies.find(c => c.id === state.activeCompanyId);
          const year = company?.financialYears.find(y => y.id === state.activeYearId);
          if (year) {
        ...
        const state = get();
        if (state.activeYearId) {
          syncToBackend(state.activeYearId, 'auditData', state.getActiveYearData().auditData);
        }
      },

      updateActiveYearNotesData: (updater: (draft: N4_13_State) => void) => {
        set(state => {
          const company = state.companies.find(c => c.id === state.activeCompanyId);
          const year = company?.financialYears.find(y => y.id === state.activeYearId);
          if (year) {
        ...
        const state = get();
        if (state.activeYearId) {
          syncToBackend(state.activeYearId, 'notesData', state.getActiveYearData().notesData);
        }
      },
