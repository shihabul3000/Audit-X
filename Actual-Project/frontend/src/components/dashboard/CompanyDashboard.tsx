...
import { useAppStore, authSelectors, permissionSelectors, getDefaultAuditData, getDefaultNotesData } from '../../store/useAppStore';
...
export const CompanyDashboard: React.FC = () => {
  ...
  const loadFinancialYears = async () => {
    ...
    try {
      ...
      // httpClient interceptor unwraps the axios response, so the array is in response.data
  ...
  const handleOpenYear = async (yearId: string) => {
    try {
      ...
      useAppStore.setState(state => {
        ...
        if (year) {
          year.data = {
            auditData: data.data.auditReportData || getDefaultAuditData(),
            notesData: data.data.notesData || getDefaultNotesData(),
          };
          if (data.data.discussionData) {
            year.data.auditData.discussionData = data.data.discussionData;
          }
        }
      });
    } catch (err) {
  ...
  const handleCreateYear = async (e: React.FormEvent) => {
    ...
    try {
      if (financialYears.length > 0) {
        const sourceYear = financialYears[0];
        await financialYearService.rollover(activeCompanyId, sourceYear.id, newDate);
      } else {
        await financialYearService.create(activeCompanyId, newDate);
      }
